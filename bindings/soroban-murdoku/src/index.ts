...
export const networks = {
  testnet: {
    ...
    contractId: "CDVKQVARXJP6HIY2MEBK2V6DPCZH4J62N3O7XVUCA4FGN2ZXKOZ4MXM6",
  }
} as const
 ...
 */
export const PuzzleError = {
  1: {message:"InvalidGridSize"},
  2: {message:"InvalidSuspects"},
  3: {message:"InvalidSolution"},
  4: {message:"InvalidClues"},
  5: {message:"PuzzleNotFound"},
...
export interface Client {
   ...
   */
  get_puzzle: ({puzzle_id}: {puzzle_id: u32}, options?: MethodOptions) => Promise<AssembledTransaction<Puzzle>>

  /**
   * Construct and simulate a start_game transaction. Returns an `AssembledTransaction` object which will have a `result` field containing the result of the simulation. If this transaction changes contract state, you will need to call `signAndSend()` on the returned object.
   * Initializes a new game session for the player.
   */
  start_game: ({player, puzzle_id}: {player: string, puzzle_id: u32}, options?: MethodOptions) => Promise<AssembledTransaction<null>>

  /**
   * Construct and simulate a list_puzzles transaction. Returns an `AssembledTransaction` object which will have a `result` field containing the result of the simulation. If this transaction changes contract state, you will need to call `signAndSend()` on the returned object.
   * Returns a paginated list of puzzle summaries (no solution field).
   */
  list_puzzles: ({offset, limit}: {offset: u32, limit: u32}, options?: MethodOptions) => Promise<AssembledTransaction<Array<PuzzleSummary>>>

  /**
   * Construct and simulate a place_suspect transaction. Returns an `AssembledTransaction` object which will have a `result` field containing the result of the simulation. If this transaction changes contract state, you will need to call `signAndSend()` on the returned object.
   */
  place_suspect: ({player, puzzle_id, x, y, suspect_idx}: {player: string, puzzle_id: u32, x: u32, y: u32, suspect_idx: u32}, options?: MethodOptions) => Promise<AssembledTransaction<MoveResult>>

  /**
   * Construct and simulate a submit_puzzle transaction. Returns an `AssembledTransaction` object which will have a `result` field containing the result of the simulation. If this transaction changes contract state, you will need to call `signAndSend()` on the returned object.
   * Validates and stores a new puzzle. Returns the assigned puzzle ID.
   */
  submit_puzzle: ({caller, grid_size, suspects, clues, solution, metadata}: {caller: string, grid_size: u32, suspects: Array<string>, clues: Array<Clue>, solution: Array<u32>, metadata: PuzzleMetadata}, options?: MethodOptions) => Promise<AssembledTransaction<u32>>

  /**
   * Construct and simulate a remove_suspect transaction. Returns an `AssembledTransaction` object which will have a `result` field containing the result of the simulation. If this transaction changes contract state, you will need to call `signAndSend()` on the returned object.
   */
  remove_suspect: ({player, puzzle_id, x, y}: {player: string, puzzle_id: u32, x: u32, y: u32}, options?: MethodOptions) => Promise<AssembledTransaction<MoveResult>>
   ...
   */
  revoke_session: ({player, puzzle_id}: {player: string, puzzle_id: u32}, options?: MethodOptions) => Promise<AssembledTransaction<null>>

  /**
   * Construct and simulate a get_player_state transaction. Returns an `AssembledTransaction` object which will have a `result` field containing the result of the simulation. If this transaction changes contract state, you will need to call `signAndSend()` on the returned object.
   * Returns the player's current game state.
   */
  get_player_state: ({player, puzzle_id}: {player: string, puzzle_id: u32}, options?: MethodOptions) => Promise<AssembledTransaction<PlayerState>>
   ...
   */
  authorize_session: ({player, puzzle_id, session_key, expires_at_ledger}: {player: string, puzzle_id: u32, session_key: Buffer, expires_at_ledger: u32}, options?: MethodOptions) => Promise<AssembledTransaction<null>>

  /**
   * Construct and simulate a deactivate_puzzle transaction. Returns an `AssembledTransaction` object which will have a `result` field containing the result of the simulation. If this transaction changes contract state, you will need to call `signAndSend()` on the returned object.
   * Allows the original creator to deactivate their puzzle. Uses ops::Ownable pattern for authorization.
   */
  deactivate_puzzle: ({caller, puzzle_id}: {caller: string, puzzle_id: u32}, options?: MethodOptions) => Promise<AssembledTransaction<null>>
...
export class Client extends ContractClient {
  ...
  constructor(public readonly options: ContractClientOptions) {
    super(
      new ContractSpec([ "",
        "",
        "",
        "",
        "",
        "AAAAAQAAADFBIHNpbmdsZSBwcmUtZmlsbGVkIGNlbGwgY2x1ZSBpbiB0aGUgcHV6emxlIGdyaWQuAAAAAAAAAAAAAARDbHVlAAAAAwAAAAAAAAADY29sAAAAAAQAAAAAAAAAA3JvdwAAAAAEAAAAAAAAAAtzdXNwZWN0X2lkeAAAAAAE",
        "AAAAAQAAACpNZXRhZGF0YSBhc3NvY2lhdGVkIHdpdGggYSBNdXJkb2t1IHB1enpsZS4AAAAAAAAAAAAOUHV6emxlTWV0YWRhdGEAAAAAAAIAAAAAAAAACmRpZmZpY3VsdHkAAAAAABAAAAAAAAAABG5hbWUAAAAQ",
        "AAAAAAAAAEBSZXR1cm5zIHRoZSBmdWxsIHB1enpsZSBkZWZpbml0aW9uIGluY2x1ZGluZyBjbHVlcyBhbmQgc29sdXRpb24uAAAACmdldF9wdXp6bGUAAAAAAAEAAAAAAAAACXB1enpsZV9pZAAAAAAAAAQAAAABAAAH0AAAAAZQdXp6bGUAAA==",
        "AAAAAAAAAC5Jbml0aWFsaXplcyBhIG5ldyBnYW1lIHNlc3Npb24gZm9yIHRoZSBwbGF5ZXIuAAAAAAAKc3RhcnRfZ2FtZQAAAAAAAgAAAAAAAAAGcGxheWVyAAAAAAATAAAAAAAAAAlwdXp6bGVfaWQAAAAAAAAEAAAAAA==",
        "AAAAAAAAAEFSZXR1cm5zIGEgcGFnaW5hdGVkIGxpc3Qgb2YgcHV6emxlIHN1bW1hcmllcyAobm8gc29sdXRpb24gZmllbGQpLgAAAAAAAAxsaXN0X3B1enpsZXMAAAACAAAAAAAAAAZvZmZzZXQAAAAAAAQAAAAAAAAABWxpbWl0AAAAAAAABAAAAAEAAAPqAAAH0AAAAA1QdXp6bGVTdW1tYXJ5AAAA",
        "AAAAAAAAAAAAAAANcGxhY2Vfc3VzcGVjdAAAAAAAAAUAAAAAAAAABnBsYXllcgAAAAAAEwAAAAAAAAAJcHV6emxlX2lkAAAAAAAABAAAAAAAAAABeAAAAAAAAAQAAAAAAAAAAXkAAAAAAAAEAAAAAAAAAAtzdXNwZWN0X2lkeAAAAAAEAAAAAQAAB9AAAAAKTW92ZVJlc3VsdAAA",
        "",
        "AAAAAAAAAAAAAAAOcmVtb3ZlX3N1c3BlY3QAAAAAAAQAAAAAAAAABnBsYXllcgAAAAAAEwAAAAAAAAAJcHV6emxlX2lkAAAAAAAABAAAAAAAAAABeAAAAAAAAAQAAAAAAAAAAXkAAAAAAAAEAAAAAQAAB9AAAAAKTW92ZVJlc3VsdAAA",
        "AAAAAAAAAAAAAAAOcmV2b2tlX3Nlc3Npb24AAAAAAAIAAAAAAAAABnBsYXllcgAAAAAAEwAAAAAAAAAJcHV6emxlX2lkAAAAAAAABAAAAAA=",
        "AAAAAAAAAChSZXR1cm5zIHRoZSBwbGF5ZXIncyBjdXJyZW50IGdhbWUgc3RhdGUuAAAAEGdldF9wbGF5ZXJfc3RhdGUAAAACAAAAAAAAAAZwbGF5ZXIAAAAAABMAAAAAAAAACXB1enpsZV9pZAAAAAAAAAQAAAABAAAH0AAAAAtQbGF5ZXJTdGF0ZQA=",
        "AAAAAAAAAC5SZXR1cm5zIHRoZSB0b3RhbCBudW1iZXIgb2Ygc3VibWl0dGVkIHB1enpsZXMuAAAAAAAQZ2V0X3B1enpsZV9jb3VudAAAAAAAAAABAAAABA==",
        "AAAAAAAAAAAAAAARYXV0aG9yaXplX3Nlc3Npb24AAAAAAAAEAAAAAAAAAAZwbGF5ZXIAAAAAABMAAAAAAAAACXB1enpsZV9pZAAAAAAAAAQAAAAAAAAAC3Nlc3Npb25fa2V5AAAAA+4AAAAgAAAAAAAAABFleHBpcmVzX2F0X2xlZGdlcgAAAAAAAAQAAAAA",
        "AAAAAAAAAGRBbGxvd3MgdGhlIG9yaWdpbmFsIGNyZWF0b3IgdG8gZGVhY3RpdmF0ZSB0aGVpciBwdXp6bGUuIFVzZXMgb3BzOjpPd25hYmxlIHBhdHRlcm4gZm9yIGF1dGhvcml6YXRpb24uAAAAEWRlYWN0aXZhdGVfcHV6emxlAAAAAAAAAgAAAAAAAAAGY2FsbGVyAAAAAAATAAAAAAAAAAlwdXp6bGVfaWQAAAAAAAAEAAAAAA==",
  ...
  public readonly fromJSON = {
    get_puzzle: this.txFromJSON<Puzzle>,
        start_game: this.txFromJSON<null>,
        list_puzzles: this.txFromJSON<Array<PuzzleSummary>>,
        place_suspect: this.txFromJSON<MoveResult>,
        submit_puzzle: this.txFromJSON<u32>,
        remove_suspect: this.txFromJSON<MoveResult>,
        revoke_session: this.txFromJSON<null>,
        get_player_state: this.txFromJSON<PlayerState>,
        get_puzzle_count: this.txFromJSON<u32>,
        authorize_session: this.txFromJSON<null>,
        deactivate_puzzle: this.txFromJSON<null>
  }
}